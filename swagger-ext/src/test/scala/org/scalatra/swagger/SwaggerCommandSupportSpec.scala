...
class SwaggerCommandSupportSpec extends MutableScalatraSpec {
  ...
  "SwaggerCommandSupport" should {
    "generate a model and parameters for a simple command" in {
      ...
      model.get.properties must containTheSameElementsAs(List("age" -> ModelProperty(DataType.Int, 0, required = false, default = Some("0")), "name" -> ModelProperty(DataType.String, 1, required = true)))
    }
    ...
    "generate a model and parameters for a full command" in {
      val parameterList = List(
        ...
        Parameter("limit", DataType.Int, Some("the max number of items to return"), paramType = ParamType.Query, defaultValue = Some("20"), required = false, position = 0),
        Parameter("skip", DataType.Int, Some("The offset for this collection index"), paramType = ParamType.Query, defaultValue = Some("0"), required = false, position = 0),
